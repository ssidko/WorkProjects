...
typedef struct _FlashParameters
{
	...
	ULONGLONG	m_nMaskID;
	BYTE		m_nVendorID;
	BYTE		m_nBusWidth;		// разрядность шины 8/16
	BYTE		m_nONFI;
	BYTE		m_nReadType;		// синхронный / ассинхронный
	BYTE		m_nCmdType;			// тип команды чтения (Page/Cache)
	BYTE		m_nNumCmd;			// 2 - default
	BYTE		m_nNumPageAdr;		// 3 - default
	BYTE		m_nNumFullAdr;		// 5 - default
	BYTE		m_nCountCS;
	BYTE		m_nCountLU;
	WORD		m_nBlockPerLU;
	WORD		m_nPagePerBlock;
	WORD		m_nPageSize;
	BYTE		m_nSpeedAsyn;		// Скорость асинхронного чтения 20, 25, 50 ... ns
	BYTE		m_nSpeedSyn;		// Скорость синхронного чтения, пока игнорится
	BYTE		m_nPowerIO;			// напряжение питания 1.8, 2.7, 3.3; 
	BYTE		m_nPowerCore;		// для чипов с двойным питанием, пока игнорится
	BYTE		m_nReserved[12];
