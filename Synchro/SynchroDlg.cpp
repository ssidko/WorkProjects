...
#define PAGES_PER_BLOCK				((WORD)128)
#define PAGE_SIZE					((WORD)4224)
#define PAGES_COUNT					((WORD)33)

	DWORD db = 12;
	DWORD ib = 1024;
	DWORD dib = 37*ib;

	DWORD all = db + ib + dib + 881; 

	CFile dump;
	
	dump.Open(_T("dump.bin"), CFile::modeCreate|CFile::modeReadWrite);

	
	DWORD nSize = 0;
	DWORD m_nPageSize = 4224;
	DWORD m_nCountLBA = 231;
	DWORD m_nCountPage = 28;

	//do
	//{
	//	m_nCountPage++;
	//	nSize = m_nPageSize * m_nCountPage;
	//	m_nCountLBA = (WORD)(nSize / 512);
	//}
	//while (nSize % 512);

	if (reader.Open(_T("\\\\.\\PhysicalDrive1")))
	{
		result = reader.PowerOn(MAKE_POWER_PARAM(PowerIO_3_3, 0, Support_No, Bus_08), buff, &rw);
		result = reader.GetFlashID(buff, &rw);
		result = reader.PowerOff(buff, &rw);


		WORD nCommandParam = MAKE_COMMAND_PARAM(5, 3, 2);
		WORD nSpeedParam = MAKE_SPEED_PARAM(SpeedAsyn_50, 0, ReadPage, 0 /* sync/async */);

		result = reader.SetFlashParameters(PAGES_PER_BLOCK, PAGE_SIZE, m_nCountPage, nCommandParam, nSpeedParam, buff, &rw);
		result = reader.SetReadParameters(MAKE_READ_PARAM(0, ReadOnce), buff, &rw);
		result = reader.PowerOn(MAKE_POWER_PARAM(PowerIO_3_3, 0, Support_No, Bus_08), buff, &rw);

		BYTE*		pReadBuf = new BYTE[m_nCountLBA*512];
		WORD		nNeedSectors = m_nCountLBA;
		WORD		nSectorsToRead;
		ULONG		nReadedBytes;


		for (DWORD nPageStart = 0; nPageStart < 128*4096; nPageStart += m_nCountPage)
		{
			DWORD nStopStatus = 0;
			nNeedSectors = m_nCountLBA;
			pReadBuf = buff;
			memset(buff, 0xCC, m_nCountLBA*512);
			
			do
			{
				nSectorsToRead = (nNeedSectors > reader.m_nMaxSectorCount) ? reader.m_nMaxSectorCount : nNeedSectors;
			//	if (!reader.ReadPage(nPageStart, nSectorsToRead, pReadBuf, &nReadedBytes))
			//	{
			//		nStopStatus = 1;
			//		break;
			//	}
			//	nNeedSectors -= nSectorsToRead;
			//	dump.Write(pReadBuf, nReadedBytes);
			//	dump.Flush();
			//} while (nNeedSectors);

				reader.ReadPage(10000, nSectorsToRead, pReadBuf, &nReadedBytes);
				int x = 90;
			} while (1);
		}
