...
BEGIN_MESSAGE_MAP(CRTMainDlg, CDialog)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_SEL_WORK_DIR_BUTTON, &CRTMainDlg::OnBnClickedSelWorkDirButton)
END_MESSAGE_MAP()
...
void CRTMainDlg::OnBnClickedSelWorkDirButton()
{
	//CFileDialog file_dlg(TRUE);
	//INT_PTR nResponse = file_dlg.DoModal();
	//if (nResponse == IDOK)
	//{
	...
	BOOL ret = frame.Create(NULL, _T("Frame Window"));
