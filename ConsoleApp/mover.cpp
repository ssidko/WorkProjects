...
BOOL SaveMovToFile(File &src_file,TCHAR *out_file_name, LONGLONG &max_size)
{
	...
	while (sizeof(MOV_HEADER) == stream.Read(buff, sizeof(MOV_HEADER))) {
		...
		if (IsValidAtom(atom)) {
			...
			atom_size = Be2Le(atom->hdr.size) - sizeof(MOV_HEADER);
			while (atom_size) {
				read_at_once = min(BUFFER_SIZE, atom_size);
...
int mover_main(TCHAR *file_name, TCHAR *out_dir)
{
	...
	if (file.Open() && PrepareDirPath(out_dir, out_dir_path))
		return mover_run(file, out_dir_path);
	return -1;
}