...
{
	sprintf_s(name, MAX_PATH, ("\\\\.\\PhysicalDrive%d"), number);
}

PhysicalDrive::PhysicalDrive(const char *drive_name) : File(drive_name)
{
	size_t sz = strlen(drive_name);
	for (int i = sz-1; i > 0; --i) {
		if (drive_name[i] == ('e')) {
			number = (DWORD)atoi(&drive_name[i+1]);
			break;
		}
	}
}
...
BOOL PhysicalDrive::Open(const char *drive_name)
{
	if (drive_name)
		strcpy_s(name, MAX_DRIVE_NAME, drive_name);
